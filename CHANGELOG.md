...
*   **版本演进**:
    *   **v4.7.6 (2026-09-19)**:
        -   **自用分支**: 基于上游 v4.7.5 的首个签名发布版本
            -   **列表接口精简**: `/v1/models`、`/v1/models/claude`、`/v1beta/models` 由 76 条别名改为 **17 条规范模型名**，三个端点共用同一出口，集合与顺序完全一致
            -   **模型级开关**: 「API 反代 → 支持模型与集成」操作列新增逐项启用/禁用；禁用即不返回，默认全部启用，保存即时生效并在重启后保持
            -   **界面精简**: 移除「中转站」推广页、导航项及其 12 个语言包文案
            -   **更新链路自持**: 更新检查与签名公钥指向本仓库，Release 由 CI 签名并产出 `updater.json`
            -   **应用标识**: 改为 `com.adolphjau.antigravity-tools`，与上游旧版可共存；数据目录仍为 `~/.antigravity_tools`
            -   **发布范围**: 仅 Windows 安装包（NSIS / MSI）与多架构容器镜像
    *   **v4.7.5 (2026-09-18)**:
