...
> 以下为本分支相对上游 `v4.7.5` 的全部改动；未列出的功能与上游一致。

### 1. `/v1/models` 系列精简为「规范模型名」

上游的三个列表接口会返回 **76 条**模型别名（同一模型存在多种写法）；本分支改为返回与界面「支持模型与集成」一致的 **17 条规范名**，三个端点共用同一出口，集合与顺序完全一致：

| 接口 | 上游 | 本分支 |
| --- | --- | --- |
| `/v1/models`（OpenAI 兼容） | 76 条别名 | **17 条规范名** |
| `/v1/models/claude`（Claude 兼容） | 76 条别名 | **17 条规范名** |
| `/v1beta/models`（Gemini 兼容） | 76 条别名 | **17 条规范名** |

> **目的**: 让「界面里看到的模型」与「接口返回的模型」一一对应，避免客户端模型下拉框被同义别名淹没。
> **兼容性**: 17 条规范名均为上游原有别名之一，映射逻辑未改动；其他上游别名仍可直接调用，只是不再出现在列表接口中。

### 2. 模型级「启用 / 禁用」开关

在 **API 反代 → 支持模型与集成** 表格的操作列新增开关：关闭某模型后，它**不再出现在上述三个列表接口**；默认全部启用。

- 保存后**即时生效**（无需重启服务/容器），重启或重新部署后设置保持
- 可用于隐藏不希望客户端看到的模型（例如只保留 Opus / Sonnet，隐藏 image 类模型）

### 3. 移除「中转站」页面

删除界面中的「中转站」页面、导航项及其 12 个语言包文案。

### 4. 更新检查与签名指向本仓库

应用内更新检查指向本仓库，不再提示上游新版本；updater 签名改用本仓库自签密钥。

### 5. 应用标识与安装方式

- 应用标识（identifier）：`com.adolphjau.antigravity-tools`，与上游 `com.lbjlaq.*` 的旧版**互不冲突、可共存**
- 数据目录仍为 `~/.antigravity_tools`，与旧版共用，账号与配置自动继承
- 安装方式只保留 **Windows 安装包** 与 **Docker 镜像** 两种（见下文「安装指南」）
